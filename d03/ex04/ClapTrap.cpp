...
ClapTrap::ClapTrap(std::string name, unsigned int hp, unsigned int maxHp, unsigned int mana, unsigned int maxMana
	, unsigned int lvl, unsigned int meleeDamage, unsigned int rangeDamage, unsigned int armor): name(name)
		, hp(hp), maxHp(maxHp), mana(mana), maxMana(maxMana), lvl(lvl), meleeDamage(meleeDamage)
		, rangeDamage(rangeDamage), armor(armor)
{
	std::cout << "CL4P-TP " << name << " has been constructed\n";
}
